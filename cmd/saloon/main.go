...
import (
	...
	"github.com/stockyard-dev/stockyard-saloon/internal/license"
	"github.com/stockyard-dev/stockyard-saloon/internal/server"
	"github.com/stockyard-dev/stockyard-saloon/internal/store"
)

func main() {
	port := getEnv("PORT", "9310")
	dataDir := getEnv("DATA_DIR", "./data")
	licenseKey := os.Getenv("SALOON_LICENSE_KEY")
	...
	log.Printf("Stockyard Saloon listening on :%s (tier: %s)", port, tier)
