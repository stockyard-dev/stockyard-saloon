...
if(q)f=f.filter(function(i){return (i.name||'').toLowerCase().includes(q)||(i.description||'').toLowerCase().includes(q)||(i.type||'').toLowerCase().includes(q)||(i.member_count||'').toLowerCase().includes(q)});
...
if(i.description)h+='<div class="item-sub">'+esc(i.description)+'</div>';
...
if(i.type)h+='<span>'+esc(i.type)+'</span>';
if(i.member_count)h+='<span>'+esc(i.member_count)+'</span>';
if(i.message_count)h+='<span>'+esc(i.message_count)+'</span>';
if(i.last_message_at)h+='<span>'+esc(i.last_message_at)+'</span>';
...
function formHTML(item){var i=item||{name:"",description:"",type:"",member_count:"",message_count:"",last_message_at:"",status:""};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+' SALOON</h2>';
...
h+='<div class="fr"><label>Member Count</label><input id="f-member-count" value="'+esc(i.member_count)+'"></div>';
h+='<div class="fr"><label>Message Count</label><input id="f-message-count" value="'+esc(i.message_count)+'"></div>';
h+='<div class="fr"><label>Last Message At</label><input id="f-last-message-at" value="'+esc(i.last_message_at)+'"></div>';
...
async function submit(){var body={name:document.getElementById('f-name').value.trim(),description:document.getElementById('f-description').value.trim(),type:document.getElementById('f-type').value.trim(),member_count:document.getElementById('f-member-count').value.trim(),message_count:document.getElementById('f-message-count').value.trim(),last_message_at:document.getElementById('f-last-message-at').value.trim(),status:document.getElementById('f-status').value.trim()};
